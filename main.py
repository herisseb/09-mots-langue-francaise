...
def mots_de_n_lettres(mots, n):
    ...
    """
    return {m for m in mots if len(m) == n}
...
def mots_avec(mots, s):
    ...
    """
    return {m for m in mots if s in m}
...
def cherche1(mots, start, stop, n):
    ...
    return {
        m for m in mots
        if len(m) == n and m.startswith(start) and m.endswith(stop)
    }
...
def cherche2(mots, lstart, lmid, lstop, nmin, nmax):
    ...
    Returns:
        ...
        lstart, contenant une chaine présente dans
        lmid et finissant par une chaine présente dans lstop, avec un nombre de
    ...
    return {
        ...
        and mot.startswith(t_start)
        and mot.endswith(t_stop)
        and any(mid in mot[1:-1] for mid in lmid)
    }
def main():
    """tests"""
    # mots = read_data(FILENAME)
